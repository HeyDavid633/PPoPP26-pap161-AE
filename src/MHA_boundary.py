...
def bert_fwd_std(mask):
    with torch.no_grad():
        ...
        for layer in range(layer_num):
            ...
            scores = rearrange(torch.baddbmm(scores, q, k_t, beta=1.0, alpha=softmax_scale),
...
if __name__ == "__main__":
    ...
    for batch_size, boundary_seq_len in test_cases:
        ...
        seq_len = boundary_seq_len
        if(model_selection == "vit_base" and boundary_seq_len >= 2048):
            seq_len = 512
        ...
        torch_compiled_breakmha_time = (t_end - t_start) * 1000 / running_iters
        print("e2e {} | bs:{} | seq:{}  |  PyTorch Compile with broken MHA boundary : {:.3f} ms / ite".format(model_selection, batch_size, boundary_seq_len, torch_compiled_breakmha_time))
