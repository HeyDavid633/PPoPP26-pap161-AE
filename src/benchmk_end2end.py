    ...
    with torch.no_grad():
        ...
        for layer in range(layer_num):
            ...
            if attention_type=="torch_attention":
                ...
                ByteTransformer_output = bytetr_attn_pybind_op(qkv_kernel[layer],qkv_bias[layer],mask,head_num)

                h =  ByteTransformer_output.view(batch_size, head_num, seq_len, head_size).permute(0, 2, 1, 3).contiguous()
    ...
    for layer in range(layer_num):
        ...
        if attention_type=="torch_attention":
            ...
            if seq_len<=256:
                ...
                test_ByteTrans = False
            ByteTransformer_output = bytetr_attn_pybind_op(vit_qkv_kernel[layer],vit_qkv_bias[layer],layer_head_mask,head_num)
...
if __name__ == "__main__":
    ...
    if(method_selection == "MCFuser" and model_selection == "vit_base"):
        print("e2e {} | bs:{} | seq:{}  |  MCFuser does not support vit_base".format(model_selection, batch_size, seq_len)) 
        exit(0)


    if(method_selection == "ByteTrans" and model_selection == "vit_base" and seq_len == 512): 
        exit(0)
    if(seq_len == 2048 and model_selection == "vit_base"):
        seq_len = 512
    ...
    if(test_Torch):
        ...
        print("e2e {} | bs:{} | seq:{}  |  Torch Native   : {:.3f} ms / iter".format(model_selection, batch_size, args.seq_len, torch_compiled_time)) 
    ...
    if(test_Torch_Compile):
        ...
        print("e2e {} | bs:{} | seq:{}  |  Torch Compile    : {:.3f} ms / iter".format(model_selection, batch_size, args.seq_len, base_time)) 
    ...
    if(test_ByteTransformer):
        ...
        print("e2e {} | bs:{} | seq:{}  |  ByteTransformer : {:.3f} ms / iter".format(model_selection, batch_size, args.seq_len, byteattn_time)) 
    ...
    if(test_MCFuser):   
        ...
        print("e2e {} | bs:{} | seq:{}  |  MCFuser : {:.3f} ms / iter".format(model_selection, batch_size, args.seq_len, MCFuser_time)) 
    ...
    if(test_STOF):    
        ...
        print("e2e {} | bs:{} | seq:{}  |  STOF            : {:.3f} ms / iter".format(model_selection, batch_size, args.seq_len, STOF_time)) 
